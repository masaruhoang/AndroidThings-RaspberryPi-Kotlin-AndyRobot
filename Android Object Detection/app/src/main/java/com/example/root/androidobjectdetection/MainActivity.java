...
public class MainActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2{
    ...
    //Handler Thread
    private Handler mFirebaseHandler = null;
    private HandlerThread mFirebanseHandlerThread = null;
    private Long mTIMER_DELAY = 1000L;
    private Rect mPointXY;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //HANDLER INIT
        mFirebanseHandlerThread = new HandlerThread("BACKGROUNDFB");
        mFirebanseHandlerThread.start();
        mFirebaseHandler = new Handler(mFirebanseHandlerThread.getLooper());
        mFirebaseHandler.post(mFirebaseRunnable);
    ...
    public void onDestroy() {
        ...
        if (mOpenCvCameraView != null)
            mOpenCvCameraView.disableView();
            mFirebanseHandlerThread.quitSafely();
            mFirebaseHandler.removeCallbacks(mFirebaseRunnable);
    }
    ...
    private void detectObject() {
        ...
        Mat kernel = new Mat(1, 50, CvType.CV_8UC1, new Scalar(255, 255, 255));
        Mat morbyte = new Mat();
        ...
        for (int ind = 0; ind < contour2.size(); ind++) {
            ...
            {    Imgproc.rectangle(mRgba, rectan3.br(), rectan3.tl(),
                    CONTOUR_COLOR);
                mPointXY = rectan3;
                Imgproc.putText(mRgba, " " + rectan3.x + " - " + rectan3.y,
                        rectan3.tl(), Core.FONT_ITALIC, 1.0 ,CONTOUR_COLOR);
                try
                {
                    mFirebaseHandler.postDelayed(mFirebaseRunnable, 100);
                }catch (Exception ex)
                {
    ...
    //Firebase Runnable
    private Runnable mFirebaseRunnable = new Runnable() {
        @Override
        public void run() {
            try {
                Thread.sleep(mTIMER_DELAY);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            mDatabase.child("PointXY").child("x").setValue(mPointXY.x);
            mDatabase.child("PointXY").child("y").setValue(mPointXY.y);
        }
    };
